...
export const loadMusicPosts = async (): Promise<PostData[]> => {
    return (await loadMarkdownFiles(`music/*.md`))
        .map(mdToPost)
        .filter((p) => p.published)
        .sort((a, b) => (b.datePublished || 0) - (a.datePublished || 0));
}